...
// Ensure this CSS file exists in your directory or update the path
import './request.css'; 
...
export default function ORequest() {
  ...
  const [preferredArea, setPreferredArea] = useState<any>(null);
  ...
  const validateForm = () => {
    ...
    else if (isNaN(Number(price)) || Number(price) <= 0) validationErrors.price = 'Enter a valid monthly price';
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    // In a real app, you'd use a FormData object or an API call here
  ...
  const handleCancel = () => {
    router.push('/owners/dashboard'); // Adjusted path for Next.js
  };
      ...
      <Row className="justify-content-center">
        <Col lg={8}>
          <div className="o-request-card p-4 shadow-sm bg-white rounded">
            <h2 className="mb-4 text-dark">Post a Property Listing</h2>
            ...
            <Form onSubmit={handleSubmit} noValidate>
              ...
              <Form.Group className="mb-4" controlId="photos">
                ...
                />
                {photos.length > 0 && (
                  <div className="photo-list mt-2 d-flex flex-wrap gap-2">
                    {photos.map((photo, index) => (
                      <div key={index} className="photo-chip border p-1 rounded bg-light d-flex align-items-center">
                        <span className="photo-name small me-2">{photo.name}</span>
                        <button
                          type="button"
                          className="btn-close"
